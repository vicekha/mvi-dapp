...
contract EulerLagrangeOrderProcessor is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Update order fill status (for partial fills)
     * @param orderId Order ID
     * @param amountFilled Amount filled in this transaction (in terms of amountOut)
     */
    function updateOrderFill(bytes32 orderId, uint256 amountFilled) external {
        ...
        if (order.filledAmount == order.amountOut) {
            ...
            order.status = OrderStatus.PARTIALLY_FILLED;
            emit OrderPartiallyFilled(orderId, order.filledAmount, order.amountOut - order.filledAmount, block.timestamp);
        }

        // Reduce liquidity proportional to the filled amount
        if (filledMinutes > 0) {
            liquidityPool.reduceLiquidity(order.tokenIn, order.tokenOut, filledMinutes);
        }
    }
    ...
    function findMatchingOrder(
        ...
        for (uint256 i = 0; i < candidates.length; i++) {
            ...
            // Skip if not active/partially filled or expired
            if ((candidate.status != OrderStatus.ACTIVE && candidate.status != OrderStatus.PARTIALLY_FILLED) || 
                block.timestamp > candidate.expiration) {
                continue;
            }

            // Target Chain Compatibility Check:
            // The candidate must be willing to match on THIS chain (targetChainId == 0 or targetChainId == current)
            if (candidate.targetChainId != 0 && candidate.targetChainId != block.chainid) {
                continue;
            }
            
            // Calculate REMAINING amounts for partially filled orders
            uint256 candidateRemainingIn;
            uint256 candidateRemainingOut;
            
            if (candidate.filledAmount > 0) {
                // Partially filled order - calculate what's left
                candidateRemainingOut = candidate.amountOut - candidate.filledAmount;
                // Proportional calculation for remaining input
                candidateRemainingIn = (candidateRemainingOut * candidate.amountIn) / candidate.amountOut;
            } else {
                // Unfilled order - use full amounts
                candidateRemainingIn = candidate.amountIn;
                candidateRemainingOut = candidate.amountOut;
            }
            
            // Check compatibility based on asset types (using REMAINING amounts)
            bool isMatch = false;
            
            // Case 1: Both sides are fungible tokens (ERC20/native)
            if (candidate.typeIn == AssetType.ERC20 && candidate.typeOut == AssetType.ERC20) {
                // PARTIAL FILL MATCHING: Match if there's ANY overlap
                // - Candidate offers candidateRemainingIn (we want amountOut of this)
                // - Candidate wants candidateRemainingOut (we offer amountIn of this)
                // Match if: candidate offers SOMETHING we want AND we offer SOMETHING candidate wants
                // i.e., both candidateRemainingIn > 0 and candidateRemainingOut > 0
                // AND there's a positive trade amount possible
                
                // Check if the exchange rates are compatible:
                // For a match: (candidateRemainingIn / candidateRemainingOut) should be >= (amountOut / amountIn)
                // Cross-multiply to avoid division: candidateRemainingIn * amountIn >= amountOut * candidateRemainingOut
                bool ratesCompatible = (candidateRemainingIn * amountIn >= amountOut * candidateRemainingOut);
                
                isMatch = ratesCompatible && candidateRemainingIn > 0 && candidateRemainingOut > 0;
            }
            // Case 2: Candidate offers NFT, wants fungible token. We have tokens, want NFT.
            else if (candidate.typeIn == AssetType.ERC721 && candidate.typeOut == AssetType.ERC20) {
                // NFTs cannot be partially filled, so if status is PARTIALLY_FILLED, skip
                if (candidate.status == OrderStatus.PARTIALLY_FILLED) continue;
                // Match if candidate has the NFT we want (exact tokenId) AND candidate wants <= what we offer
                isMatch = (candidateRemainingIn == amountOut && candidateRemainingOut <= amountIn);
            }
            // Case 3: Candidate offers fungible, wants NFT. We have NFT, want tokens.
            else if (candidate.typeIn == AssetType.ERC20 && candidate.typeOut == AssetType.ERC721) {
                // NFTs cannot be partially filled, so if status is PARTIALLY_FILLED, skip
                if (candidate.status == OrderStatus.PARTIALLY_FILLED) continue;
                // Match if candidate offers >= what we want AND candidate wants the NFT we have (exact tokenId)
                isMatch = (candidateRemainingIn >= amountOut && candidateRemainingOut == amountIn);
            }
            // Case 4: Both sides are NFTs
                ...
                // NFTs cannot be partially filled, so if status is PARTIALLY_FILLED, skip
                if (candidate.status == OrderStatus.PARTIALLY_FILLED) continue;
                // NFT for NFT swap - match by exact tokenIds for BOTH sides
                isMatch = (candidateRemainingOut == amountIn && candidateRemainingIn == amountOut);
            }
    ...
    /**
     * @dev Get order queue logic removed
     */
