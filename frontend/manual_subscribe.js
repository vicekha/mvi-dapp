...
async function subscribe() {
    ...
    // Use staticNetwork to avoid ENS lookups
    const network = {
        name: 'lasna',
        chainId: 5318007
    };

    const provider = new ethers.JsonRpcProvider('https://lasna-rpc.rnk.dev/', network, {
    ...
    try {
        ...
        if (error.message.includes('Unauthorized')) {
            ...
            console.error('Deployer needs more gas (LREACT) to send the transaction.');
        }
        throw error;
    }
}
