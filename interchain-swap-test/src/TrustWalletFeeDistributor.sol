    ...
    function distributeFee(
        ...
        if (fee > 0) {
            ...
            if (token == address(0) || assetType == AssetType.ERC721) {
                ...
                // Keep fee in contract for debt coverage
                accumulatedFees[address(0)] += fee;
            } else {
                // For ERC20, we pull it in token units
            ...
            feeHistory.push(
                FeeRecord({
                    token: token, recipient: address(this), amount: fee, timestamp: block.timestamp, orderId: orderId
                })
            );
